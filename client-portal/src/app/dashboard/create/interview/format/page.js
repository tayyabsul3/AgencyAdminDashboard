...
import styles from '../generate/ArticleGeneration.module.css';

// Force dynamic rendering to avoid prerendering issues with useSearchParams
export const dynamic = 'force-dynamic';

function FormatArticlePageContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await getInterviewById(user.uid, slug);
    ...
    return (
      <div className={styles.errorContainer}>
        <div className={styles.errorMessage}>
          ...
          <p>{error}</p>
          <button className={styles.errorButton} onClick={() => router.back()}>Go Back</button>
        </div>
      </div>
    );
  }
      ...
      <div className="container" style={{ width: '100%', margin: 0, padding: 0 }}>
        <div className={styles.generationCard} style={{ width: '100%', maxWidth: 'none', borderRadius: 0, margin: 0 }}>
          ...
          <div id="top" className={styles.previewHeader}>
            <h1 className={styles.articleTitle}>🏆 {article?.topic || topic}</h1>
            <div className={styles.previewActions}>
              <button
                className={styles.secondaryButton}
                onClick={() => router.push(`/dashboard/create/interview/generate?id=${encodeURIComponent(slug)}&topic=${encodeURIComponent(topic)}`)}
              >
                ← Back to Generate
              </button>
              <button
                className={styles.secondaryButton}
                onClick={() => router.push(`/dashboard/articles/view?id=${encodeURIComponent(slug)}`)}
                title="Open this article detail view"
              >
                View Live Article
              </button>
            </div>
          ...
          <div className={styles.previewContent} style={{ maxHeight: 'none', overflow: 'visible' }}>
                ...
                <p className={styles.sectionParagraph}>
                  {(() => {
                    ...
                    const introText = Object.values(intro)
                      ...
                      .join(' ');
                    return introText || `This expert guide on ${article?.topic || topic} compiles your interview answers into a structured FAQ.`;
                  })()}
                </p>
              </div>
            </div>
            ...
            <div id="toc" className={styles.previewSection}>
              <h3>📑 {article?.topic || topic} – Table of Contents</h3>
              <div className={styles.sectionPreview}>
            ...
            <div className={styles.previewSection}>
              ...
              <p className={styles.authorBio}>
                {(() => {
                  // Prefer generated author bio from final article when present
                  ...
                  const intro = article?.setup?.expertIntro || {};
                  const topicText = article?.topic || article?.setup?.topic || topic || 'this topic';
