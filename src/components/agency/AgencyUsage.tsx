...
const dispatch = useAppDispatch();
  ...
  const handleCustomPackageSelect = (credits: number) => {
    const customPackage: CreditPackage = {
      credits: credits,
      price: credits * 25, // $25 per credit
      popular: false,
      name: "Custom Pack"
    };
  ...
  const createPaymentRecord = async () => {
    try {
      ...
      const paymentRecord = {
        ...
        amount: selectedPackage?.price || creditAmount * 25,
        status: 'pending',
        createdAt: new Date(),
        updatedAt: new Date()
      };

      const docRef = await addDoc(collection(db, "Addons"), paymentRecord);
      setPaymentRecordId(docRef.id);
      return docRef.id;
    } catch (error) {
  ...
  const handleProceedPayment = async () => {
    ...
    const payId = await createPaymentRecord();
    if (payId) {
      setShowPaymentElement(true);
    }
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
         ...
         {!(agencyData?.subscription?.tier === "CustomAgency") && <AgencyUpgrade 
            agencyData={agencyData} 
            userType={userType}
            onTierUpdate={refreshAgencyData} // Use the refresh function
          />}
          <button
            onClick={() => {
              openAddCreditsModal();
            ...
            <div>
              ...
              {usedSeats > subscription.seats && (
                <p className="text-red-600 text-sm mt-1 font-medium">
                  ⚠️ Over limit by {usedSeats - subscription.seats}
                </p>
              )}
            </div>
          ...
          <div>
            ...
            {subscription.stripeSubscriptionId && (
              <p className="text-gray-500 text-sm mt-1">
                Subscription ID: {subscription.stripeSubscriptionId.substring(0, 8)}...
              </p>
            )}
          </div>
      ...
      {/* Add Credits Modal */}
      <Modal isOpen={isAddCreditsOpen} onClose={handleCloseModal} className="max-w-4xl max-h-[95dvh] overflow-auto p-8">
        ...
        {!showPaymentElement ? (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div>
              ...
              {/* Custom Amount */}
              <div className="mt-6">
                <label className="block font-semibold text-gray-900 mb-2">Or create custom package</label>
                <div className="flex gap-3">
                  <input
                    type="number"
                    min="1"
                  ...
                  />
                  <button
                    onClick={() => handleCustomPackageSelect(creditAmount)}
                ...
                <p className="text-gray-500 mt-2">
                  Price: ${(creditAmount * 25 )} for {creditAmount} credits
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-bold text-gray-900">
                    {selectedPackage ? selectedPackage.name : 'None selected'}
                  </span>
                </div>
...
>
  Proceed to Payment - ${(creditAmount * 25)}
</button>
         ...
         <div className="p-6 shadow-2xl rounded-2xl border border-gray-200">
    <div className="mb-6">
      <h4 className="font-bold text-gray-900 text-xl mb-2">Complete Your Purchase</h4>
      <p className="text-gray-600">
  ...
  className="w-full mt-4 py-3 text-gray-600 bg-gray-100 font-semibold rounded-lg hover:bg-gray-200 transition-colors"
>
